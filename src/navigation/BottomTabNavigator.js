...
const CustomTabBar = ({ state, descriptors, navigation }) => {
    ...
    return (
      <View style={styles.tabBarContainer}>
          ...
          {mainTabs.map((route, index) => {
            ...
            return (
              <TouchableOpacity key={index} onPress={onPress} style={styles.tabButton}>
                <Ionicons name={isFocused ? iconName : `${iconName}-outline`} size={24} color={isFocused ? '#000' : '#999'} />
                <Text style={{ color: isFocused ? '#000' : '#999', fontSize: 10, marginTop: 4, fontWeight: isFocused ? '600' : '400' }}>
                  {label}
                </Text>
              </TouchableOpacity>
            );
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Search')} style={styles.searchButtonContainer}>
...
const styles = StyleSheet.create({
    tabBarContainer: { position: 'absolute', bottom: 30, left: 20, right: 20, height: 65, flexDirection: 'row', alignItems: 'center' },
    blurPill: { flex: 1, flexDirection: 'row', height: '100%', borderRadius: 50, overflow: 'hidden', backgroundColor: 'rgba(255,255,255,0.3)' },
    tabButton: { flex: 1, justifyContent: 'center', alignItems: 'center' },
    searchButtonContainer: { width: 60, height: 60, borderRadius: 30, marginLeft: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 5, elevation: 10 },
    searchButtonBlur: { width: '100%', height: '100%', borderRadius: 30, justifyContent: 'center', alignItems: 'center', overflow: 'hidden', backgroundColor: 'rgba(255,255,255,0.4)' }
});
