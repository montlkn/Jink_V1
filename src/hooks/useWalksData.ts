...
import {
  ...
  toWalkSummaryList,
  type WalkDetail,
  type WalkSummaryItem,
} from "@/features/walks/selectors";
import {
  getDemoWalkGeometry,
...
const fetchSummariesWithCache = async (
  ...
  try {
    ...
    log.warn(
      "[walks] Failed to load walk summaries",
...
export function useWalksData(): WalksDataState {
  ...
  const loadWalkData = useCallback(
    async (options?: LoadOptions): Promise<WalksReadyValue> => {
      ...
      const requestedWalkId = options?.walkId ?? selectedWalkIdRef.current ??
        null;
      ...
      const resolvedWalkId = options?.walkId ??
      ...
      selectedWalkIdRef.current = normalizedSelected?.walkId ??
        resolvedWalkId ?? null;
