...
BEGIN
  ...
  -- Case 4: Missed a day (with 48h grace period for offline reconciliation)
  ELSIF v_last_activity < v_today - INTERVAL '2 days' THEN
  ...
  -- Case 5: Within grace period (missed 1 day but within 48h)
  ELSE
    v_current_streak := 1;
    v_is_new_day := true;
...
'Updates user daily streak based on last activity date. Returns current streak count, whether this is a new day, and the previous streak count.';
