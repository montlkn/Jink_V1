...
var withMapsIOS = function (config, props) {
    ...
    // Add Cocoapods for RN Maps and (optionally) Google.
    config = withMapsCocoaPods(config, {
        useGoogleMaps: !!(props === null || props === void 0 ? void 0 : props.iosGoogleMapsApiKey),
    });
    // Attempt to add AppDelegate init; if unsupported language, skip instead of throwing.
...
var withGoogleMapsAppDelegate = function (config, _a) {
    ...
    return (0, ios_plugins_1.withAppDelegate)(config, function (conf) {
        ...
        if (conf.modResults.language !== 'swift') {
            // Skip modification for non-Swift AppDelegate (e.g., ObjC/ObjC++). Info.plist key will be used.
            try {
                console.warn('[react-native-maps] Skipping AppDelegate modification (unsupported language: ' + conf.modResults.language + ').');
            }
            catch (_b) { }
            return conf;
        }
