import { normalize, type AestheticBreakdown } from "./normalize";
...
export function totalVariation(
  oldMap: AestheticBreakdown | null | undefined,
  newMap: AestheticBreakdown | null | undefined
): number {
...
export function checkDivergence(
  currentBreakdown: AestheticBreakdown,
  baselineBreakdown: AestheticBreakdown
): { shouldRegenerate: boolean; divergencePct: number; thresholdPct: number } {
  const normalizedCurrent = normalize(currentBreakdown);
  const normalizedBaseline = normalize(baselineBreakdown);

  const divergence = totalVariation(normalizedBaseline, normalizedCurrent);
...
export function shouldUpdateBaseline(
  currentBreakdown: AestheticBreakdown,
  baselineBreakdown: AestheticBreakdown,
  baselineGeneratedAt: Date | string,
  interactionsSinceBaseline: number
): boolean {
...
export function createBaselineSnapshot(
  ...
  baseline_version: string | number;
  baseline_breakdown: AestheticBreakdown;
} {
  return {
